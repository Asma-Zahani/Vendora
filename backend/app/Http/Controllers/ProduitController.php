...
class ProduitController extends Controller implements HasMiddleware
{
    ...
    {
        $produits = Produit::with('couleurs')->where('status', StatusProduitEnum::Disponible)->latest('created_at')->take(8)->get();
        return response()->json($produits);
    }
    ...
    {
        $produit = Produit::with(['couleurs', 'sousCategorie.categorie'])->find($id);
