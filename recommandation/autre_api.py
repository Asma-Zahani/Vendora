...
def get_filtered_interaction_matrix(interaction_type):
    if df_interactions.empty:
        ...
        df_interaction = df_interactions[df_interactions["interaction_type"] == interaction_type]
    
        interaction_matrix = df_interaction.pivot_table(index="user_id", columns="produit_id", aggfunc="size", fill_value=0)
...
def recommend_similar_products():
    ...
    interaction_type = data.get("interaction_type", "panier")

    if product_id is None:
        return jsonify({"error": "Le produit est requis"}), 400
    
    product_similarity_df = get_filtered_interaction_matrix(interaction_type)
