...
const EntityManager = ({filtres, columns, fields, label, identifiant, formData, setFormData, actionList, notAdd , data}) => {
  ...
  useEffect(() => {
    if (!data) {
      fetchData(
        ...
        sortOrder,
        setEntities,
        { [filtres?.field]: filtres?.selectedFilter }
      );
    } else {
      setEntities({ data });
    }
  }, [data, currentPage, selectedItemPerPage, searchTerm, sortBy, sortOrder, filtres?.selectedFilter]);
