...
const Horaires = () => {
  ...
  const [formData, setFormData] = useState({ periode_id: '', horaire_id: '', heureDebut: '', heureFin: '' });
                  ...
                  <div className="overflow-x-auto scrollbar">
                        ...
                        {selectedDrive.horaires.map((item) => (
                          <tr key={item.horaire_id} className="border-b border-contentLight dark:border-borderDark">
                            ...
                            <td className="py-4 text-sm whitespace-nowrap px-4 space-y-2">
                              {item.periodes && item.periodes.length > 0 &&
                                item.periodes.map((periode, index) => {
                                  if (item.ouvert) { 
                                    return (
                                      <div key={index} className="flex items-center space-x-2">
                                        ...
                                        <button onClick={() => {setFormData({ periode_id: periode.periode_id, horaire_id: periode.pivot.horaire_id}); setIsDeleteOpen(true)}} className="text-gray-500 transition-colors duration-200 dark:hover:text-red-500 dark:text-gray-300 hover:text-red-500 focus:outline-none">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-5 h-5">
                    ...
                    {isModalOpen && (
                      <FormModal 
                        ...
                        action={formData.periode_id ? "Modifier" : "Ajouter"}
                        formData={formData} 
                        setFormData={setFormData} 
                        errors={errors}
                        fields={[
                          { label: "Heure de début", key: "heureDebut", type: "time", value: formData.heureDebut?.slice(0, 5) },
                          { label: "Heure de fin", key: "heureFin", type: "time", value: formData.heureDebut?.slice(0, 5) },
                        ]}
                        onSubmit={async () => { 
                          let isValid = false;
      
                          if (formData.periode_id) {
                            isValid = await handleEdit("periodes", "periode_id", formData, setErrors, setSuccessMessage);
                          } else {
                            isValid = await handleCreate("periodes", formData, setErrors, setSuccessMessage);
                          }
