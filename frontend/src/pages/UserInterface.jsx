...
const UserInterface = () => {
  ...
  return (
    <div className="text-base dark:text-zinc-100 min-h-screen flex flex-col relative">
      ...
      {isVisible && ( <button onClick={() => { window.scrollTo({ top: 0, behavior: "smooth" })}} className="fixed bottom-[122px] sm:bottom-18 right-4 bg-contentLight dark:bg-contentDark border border-purpleLight text-purpleLight p-3 rounded-lg shadow-lg transition-all transform hover:scale-110 z-10">
          <ChevronsUp size={25} />
      </button>)}
    </div>
  );
