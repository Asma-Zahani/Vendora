...
const Dropdown = ({ label, name, options, selectedValue, onSelect, isOpen, toggleOpen, error, target }) => {
  const selectedOption = options.find(option => option.value === selectedValue);
  return (
    <div className={`${target ? "mb-4" : ""}`}>
      <div className="relative">
        {target && <Label label={label} />}
        <Input value={selectedOption ? `${selectedOption.label}` :  `Select a ${label}`} isOpen={isOpen} name={name} onClick={toggleOpen} error={error} required readOnly/>
