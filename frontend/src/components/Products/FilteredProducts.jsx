...
const FilteredProducts = ({ user, datas, gridInfo, filtres, ajouterAuPanier, ajouterAuListeSouhait }) => {
            ...
            <div className={`mt-10 gap-6 
                ...
                {datas.length > 0 && currentItems.length > 0 ? (
                    currentItems.map((produit, index) => (
                        gridInfo.isGrid ? (
                            <Card key={index} user={user} produit={produit} ajouterAuPanier={ajouterAuPanier} ajouterAuListeSouhait={ajouterAuListeSouhait} />
                        ) : (
